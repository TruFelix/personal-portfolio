...
const Knob: React.FC<KnobProps> = ({ label, onChange, value: inputValue, step, min, max, width, height }) => {
  ...
  return (
    <div
      ...
      onWheel={handleMouseWheel} onMouseDown={handleMouseDown} onTouchStart={handleTouchStart}
    >
      <div style={{
        position: "absolute",
        ...
        fontSize: "0.7rem",
        lineHeight: "0.7rem",
      }}>{displayValue?.toPrecision(2).toString()}</div>
      <KnobMain position={position} height={height} />
      <div style={{
        textAlign: "center",
        margin: 0,
        marginTop: "-0.5rem",
...
const KnobMain: React.FC<KnobMainProps> = ({ position, height }) => {
